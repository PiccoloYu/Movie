		...
		'autocomplete': function(){
			...
			$('#wd').keyup(function(){
				...
				$.ajax({
					...
					beforeSend:function(){
						$('#word').append('<div class="autocomplete-suggestion">姝ｅ湪鍔犺浇銆傘€傘€�</div>');
					},
					success:function(data){
						$('#word').empty().show();
						if (data.s=='')
						{
							$('#word').append('<div class="autocomplete-suggestion">鏈壘鍒颁笌 "' + keywords + '"鐩稿叧鐨勭粨鏋�</div>');
						}
					...
					error:function(){
						$('#word').empty().show();
						$('#word').append('<div class="autocomplete-suggestion">鏌ユ壘"' + keywords + '"澶辫触</div>');
					}
				})
			})
			//鐐瑰嚮鎼滅储鏁版嵁澶嶅埗缁欐悳绱㈡
			$(document).on('click','.autocomplete-suggestion',function(){
				...
				$('#wd').val(word);
				$('#word').hide();
				$('#submit').trigger('click');瑙﹀彂鎼滅储浜嬩欢
			})		
			//澶卞幓鐒︾偣鏃堕殣钘�	
...
$(document).ready(function() {
	...
	//閫氳繃鍒ゆ柇瀵艰埅"鐢靛奖"閫変腑鏉ヨ鎾斁璧勬簮鎸夐挳鎵╁20%
    var ismove = $('.stui-header__menu li:eq(1)').hasClass('active');
    if( ismove == true ){
	$('.stui-content__playlist.column10 li').css({ "width":"20%" });
    }

});
if(navigator.userAgent.match(/(iPhone|iPod|ios|Android)/i)){
